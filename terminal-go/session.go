...
func (s *Session) broadcastData(data []byte) {
    ...
    isEcho := false
    originalSource := ""
    if s.lastInputSource != "" && time.Since(s.lastInputTime) < 100*time.Millisecond {
        isEcho = true
        originalSource = s.lastInputSource
    }

    if s.eventHandler != nil {
        s.eventHandler.OnTerminalData(s.ID, data, seqNum, isEcho, originalSource)
    } else {
        s.config.logger.Warn("No event handler for terminal data", "sessionID", s.ID)
    }
}
