...
impl BeamtermRenderer {
    ...
    /// Create a terminal renderer with a dynamic font atlas using a direct canvas
    /// reference. This remains correct while a component-owned canvas is mounted in
    /// a detached DOM subtree and avoids document-wide selector lookup entirely.
    #[wasm_bindgen(js_name = "withDynamicAtlasCanvas")]
    pub fn with_dynamic_atlas_canvas(
    ...
    /// Set the background color used for canvas padding outside the terminal grid.
