...
- feature 开发：一个 feature = 一个 worktree 目录 + 一个分支；目录放在父目录，命名统一使用 `../floeterm-feat-<topic>`，分支命名统一使用 `feat-<topic>`。
...
- `make check` 会覆盖 Go/前端的核心门禁（例如 go test -race、govulncheck、web lint/test/build、npm audit 等）。

---

## 3. Release / Tag 规则

- 本仓库发布使用 tag：`v<version>`（例如 `v0.4.1`）。
